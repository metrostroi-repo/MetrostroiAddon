...
--------------------------------------------------------------------------------
Metrostroi.DefineSystem("PUAV_N")
...
function TRAIN_SYSTEM:Outputs()
    return { "Ring","RingZero","LK16" , "LAVT", "LOS", "LRS", "LKI1", "LKI2", "NoFreq", "F5", "F4", "F3", "F2", "F1" }
end
...
function TRAIN_SYSTEM:Think(dT)
    ...
    local speed = math.Round(Train.ALS_ARS.Speed or 0,1)
    ...
    if Power and not BackReverser and self.Checked==true and Reverser then
        ...
        if self.BrakeProgramm then
            ...
            if AV > 0 then
                ...
                if self.TargetKPRK and self.TargetKPRK > 1 and self.KPRK < self.TargetKPRK-0.3 then
                    local diff = self.TargetKPRK-self.LastKRPK
                    if diff <= 1.4 then
                        self.VPR = CurTime()
                    else
                        self:SetDriveMode("AT")
                    end
                elseif self.TargetKPRK == 5 then
        ...
        local CanOpenLeft = CanOpen and (self.DoorsLeft or self.ALSPower*NoFreq > 0)
        local CanOpenRight = CanOpen and (self.DoorsRight or self.ALSPower*NoFreq > 0)
        ...
        if self.OpenLeftTimer and CurTime()-self.OpenLeftTimer > 1 then self.OpenLeftTimer = nil end
        if self.OpenRightTimer and CurTime()-self.OpenRightTimer > 1 then self.OpenRightTimer = nil end
