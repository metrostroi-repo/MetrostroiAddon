...
function TRAIN_SYSTEM:Initialize(parameters)
    -- Реле дверей (РД)
    self.Train:LoadSystem("RD","Relay","REV-821",{ close_time = 0.1, coil_res = 448, trigger_level = 0.002 })
    -- Реле включения освещения (РВО)
    self.Train:LoadSystem("RVO","Relay","REV-814T",{ open_time = 4.0, coil_res = 21, trigger_level = 0.042 })
    -- Реле времени торможения (РВ3)
    self.Train:LoadSystem("RV3","Relay","REV-813T",{ open_time = 2.3, coil_res = 244, trigger_level = 0.004 })
    -- Реле тока (РТ2)
    ...
    self.Train:LoadSystem("RKTT","Relay","R-52B", { coil_res = 129, trigger_level = 0.008 })
    self.Train.RKTTsh = 1
end
