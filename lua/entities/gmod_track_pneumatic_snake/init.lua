...
function ENT:Initialize()
	...
	if IsValidPhysObj(physobj) then physobj:SetMass(50) end
end
function ENT:Touch(ent)
	if IsValidEnt(ent) and (ent:GetClass() == "gmod_train_bogey" and not ent.DontHaveCoupler or ent:GetClass() == "gmod_train_couple") then
		local pos = ent:WorldToLocal(self:GetPos())
		local cpos = ent.CouplingPointOffset
		local X = cpos.x - 13
		if not self.Timer and pos.x > X and self.CoupledWith ~= ent and not IsValidEnt(ent.DepotPneumo) then
			...
			if IsValidEnt(constraint.Weld(ent,self,0,0,0--[[33000]],true,false)) then
...
function ENT:Use(ply) --TODO: GravGunPickup
	if IsValidEnt(self.CoupledWith) then
...
function ENT:Think()
	...
	if IsValidEnt(self.CoupledWith) and self.CoupledWith.DepotPneumo == self then
		local coupled = false
