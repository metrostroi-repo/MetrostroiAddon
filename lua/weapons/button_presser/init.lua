...
function SWEP:Think()
	...
	if self.PrimAttack and IsValidEnt(self.Entity) and self.Entity:GetClass() == "momentary_rot_button" then
	...
	if self.PrimAttack ~= self.OldPrimAttack then
		...
		if trace.Hit and rot ~= nil and not IsValidEnt(self.Entity) then
			self.Entity = trace.Entity
			if not rot then
				self.Entity:Fire("Press")
			end
		elseif not self.PrimAttack and IsValidEnt(self.Entity) then
			self.Entity = nil
		end
	...
	local trace = util.TraceLine( tr )
	if not trace.Hit or not IsValidEnt(trace.Entity) then
